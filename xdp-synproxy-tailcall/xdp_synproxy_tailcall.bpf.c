...
} tail_call_tbl SEC(".maps") = {
        .values = {
                [PROG_SYNPROXY_INDEX] = (void *)&syncookie_xdp,
        },
};

// Main XDP program
SEC("xdp")
int synproxy_tailcall(struct xdp_md *ctx) {
    void *data_end = (void *)(long)ctx->data_end;
    void *data = (void *)(long)ctx->data;
    ...
    if (ip->protocol == IPPROTO_TCP) {
        ...
        // Tail call the SYNPROXY program
      	bpf_tail_call(ctx, &tail_call_tbl, PROG_SYNPROXY_INDEX);
