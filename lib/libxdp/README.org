...
defaults to =/usr/lib/bpf= (alternatively =/usr/lib64/bpf= or =/usr/local/lib/bpf=,
depending on which =PREFIX= libxdp was compiled with). This is convenient
for applications shipping pre-compiled eBPF object files.

When libxdp is compiled with the DEBUG flag, it will additionally look
for BPF object files in the current directory, before checking the
system-wide directory. This can be convenient during development to avoid
older versions of a BPF program being loaded. However, it should not be used in
production, as the shadowing of system BPF programs can be a security issue.
...
AF_XDP sockets can be created tied to this umem with
xsk_socket__create_opts(). If neither fill nor comp is set, it behaves
like xsk_socket__create() and the socket takes exclusive ownership of
the umem; if both are set, it behaves like xsk_socket__create_shared()
and the umem is shared with other sockets. There is one option called
...
In the same way, xsk_socket__create_opts() is the recommended way of socket
creation: instead of the separate arguments that xsk_socket__create() and
xsk_socket__create_shared() take, the ring configuration and flags are passed
in a struct xsk_socket_opts.
...
xsk_socket__create_opts() with created umem.  xsk_socket__create_shared() would
