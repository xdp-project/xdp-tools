...
int load_dispatcher_v1(int ifindex)
{
	...
	ret = bpf_program__set_attach_target(
		xdp_prog, bpf_program__fd(dispatcher_prog), attach_func);
	if (ret)
		goto out;
...
int load_dispatcher_v2(int ifindex)
{
	struct xdp_dispatcher_config_v2 dispatcher_config = {};
	struct bpf_object *obj_dispatcher, *obj_prog = NULL;
	DECLARE_LIBBPF_OPTS(bpf_link_create_opts, opts);
	struct bpf_program *dispatcher_prog, *xdp_prog;
	int ret, btf_id, lfd = -1, dispatcher_id;
	...
	obj_dispatcher = bpf_object__open("xdp_dispatcher_v2.o");
	if (!obj_dispatcher)
		return -errno;
	...
	dispatcher_config.magic = XDP_DISPATCHER_MAGIC;
	dispatcher_config.num_progs_enabled = 1;
	dispatcher_config.chain_call_actions[0] = PROG_CHAIN_CALL_ACTIONS;
	dispatcher_config.run_prios[0] = PROG_RUN_PRIO;
	dispatcher_config.is_xdp_frags = 0;
	dispatcher_config.program_flags[0] = 0;
	dispatcher_config.dispatcher_version = XDP_DISPATCHER_VERSION_V2;
...
int check_old_dispatcher(int ifindex, unsigned int dispatcher_version)
{
	...
	case XDP_DISPATCHER_VERSION_V2:
		ret = load_dispatcher_v2(ifindex);
		break;
	default:
		ret = -EINVAL;
		goto out;
