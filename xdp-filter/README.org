...
** -d, --deny-default
This option causes =xdp-filter= to run in /deny by default/ mode instead of the
default /allow by default/ mode. In /deny by default/ mode, *all* packets are
dropped *except* those matched by the filter options, whereas in /allow by
default/ mode, only the packets matched by the specified rules are dropped.

=xdp-filter= cannot be loaded simultaneously in /deny by default/ and /allow by
default/ mode on the system. Note that loading =xdp-filter= in this mode will
drop all traffic on the interface until suitable allow rules are installed.
