...
programs loaded on the interface. Use the -D option to see the loaded programs.
If you have multiple programs loaded with the same name, you can add the
programs index to it with the =@<index>= prefix. The index starts with 0 being
the dispatcher. See the Examples section for an example.
...
and supply it with this option.
** -P, --promiscuous-mode
...
#+begin_src
# xdpdump -i eth0 --rx-capture=entry,exit -p xdp_dispatcher,xdp_pass@2
listening on eth0, ingress XDP program ID 4311 func xdp_dispatcher, capture mode entry/exit, capture size 262144 bytes
