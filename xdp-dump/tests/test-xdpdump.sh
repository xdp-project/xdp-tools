...
test_multi_prog()
{
    ...
    PID=$(start_background "$XDPDUMP -i $NS -p xdp_dispatcher,xdp_pass@3")
    $PING6 -W 2 -c 1 "$INSIDE_IP6" || return 1
    RESULT=$(stop_background "$PID")
    ...
    PID=$(start_background "$XDPDUMP -i $NS -p xdp_dispatcher,xdp_pass@3 --rx-capture=exit")
    $PING6 -W 2 -c 1 "$INSIDE_IP6" || return 1
    RESULT=$(stop_background "$PID")
    ...
    PID=$(start_background "$XDPDUMP -i $NS -p xdp_dispatcher,xdp_pass@3 --rx-capture=exit,entry")
    $PING6 -W 2 -c 1 "$INSIDE_IP6" || return 1
    RESULT=$(stop_background "$PID")
    if ! [[ $RESULT =~ $ENTRY_REGEX ]]; then
